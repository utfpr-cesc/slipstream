...
var isIE = (function() {
    ...
    if (navigator.appName == 'Microsoft Internet Explorer') {
        ...
        rv = ua.indexOf('trident/') > -1
        ? 11 : ua.indexOf('edge/') > -1 ? 12
        : 0;
    }
    return rv;
})();
...
function tryConnect() {
    log('running: nc -v ' + external + ' ' + port);
...
//    gibson = document.createElement("form")
    ...
    if (reuse) {
        gibson.setAttribute("name", "B" + parseInt(Math.random()*10).toString());
    }
    if (reuse) {
        gibson.setAttribute("id", "pinform");
    } else {
        gibson.setAttribute("id", "rndform" + rnd());
    }
...
function startStun() {
    ...
    if (pc.createDataChannel) {
        pc.createDataChannel('', { reliable: false }); // XXX is this required? maybe for some browsers
    }
